...
/**
 * Generated class for the VideoCreatePage page.
 *
...
@IonicPage({
  name: 'video-create',
  segment: 'video-create'
})
@Component({
  selector: 'page-video-create',
...
export class VideoCreatePage implements OnInit {
  ...
  checkPermission() {
    this.contentProvider.checkPermission({ spaceid: this.spaceId, entitytype: 7 })
      .finally(() => {
        this.dismiss();
      })
      .subscribe(response => {
  ...
  save() {
    if (this.videofile == undefined) {
      this.translate.get("TOAST.VIDEOEMPTY").subscribe(res => {
    ...
    this.contentProvider.createVideo(body, this.videofile)
      .then(res => {
        saveloader.dismiss();
        ...
        this.appCenterAnalytics.isEnabled().then((b) => {
          if (b) {
            this.appCenterAnalytics.trackEvent('Video Create Success.', { id: res.ResponseData, userid: localStorage.getItem('UserID') }).then(() => {
              console.log('Video Create Event tracked');
            });
          }
        })
      })
      .catch(err => {
        ...
        this.appCenterAnalytics.isEnabled().then((b) => {
          if (b) {
            this.appCenterAnalytics.trackEvent('Video Creation Failed.', { error: err, userid: localStorage.getItem('UserID') }).then(() => {
              console.log('Video Creation Failed Event F');
            });
          }
        })
      })
