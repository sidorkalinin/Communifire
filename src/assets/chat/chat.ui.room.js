(function ($, window, chat) {
    ...
    Room.prototype.hasNewMessages = function () {
        return this.tab.data('new-messages');
    };

    Room.prototype.removeNewMessagesStatus = function () {
        return this.tab.removeData('new-messages');
    };

    Room.prototype.setNewMessagesStatus = function () {
        return this.tab.data('new-messages', true);
    };

    Room.prototype.setTab = function ($tab) {
        return this.tab = $tab;
    };
    ...
    Room.prototype.isHistory = function () {
        return this.tab.hasClass('history');
    };
    ...
    Room.prototype.updateUnread = function (isMentioned) {
        var $tab = this.tab.addClass('unread'),
            $content = $tab.find('.message-count'),
            //unread = ($tab.data('unread') || 0) + 1,
            unread = (parseInt($content.text(), 0) || 0) + 1,
    ...
    Room.prototype.isLoaded = function () {
        return this.tab.attr('data-loaded') === 'true';
    };

    Room.prototype.markLoaded = function () {
        this.tab.attr('data-loaded', true);
    };
    ...
    Room.prototype.makeInactive = function () {
        ...
        this.tab.find('a').removeClass('active');
    ...
    Room.prototype.isUnread = function() {
        return this.tab.hasClass('unread');
    }
    Room.prototype.makeActive = function () {
        ...
        this.tab.find('a').addClass('active');
    ...
    Room.prototype.clearUserList = function (userViewModel, $user) {
        this.activeUsers.empty();
    };
    ...
    Room.prototype.sortList = function (listToSort, user) {
        var listItems = listToSort.children('li:not(.empty)').get(),
            userName = ($(user).data('displayname') || '').toString(),
            userActive = $(user).data('active');

        for (var i = 0; i < listItems.length; i++) {
            var otherName = ($(listItems[i]).data('displayname') || '').toString(),
