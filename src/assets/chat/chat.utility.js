...
(function ($, window, emoji, marked, hljs, linkify, moment, languageResources, cf) {
    "use strict";
    ...
    function processMarkdown(src) {
        return cfUtilities.htmlEncode(src);
    }
    ...
    $.fn.expandableContent = function () {
        ...
        var selectors = ['img:not(.gravatar),.file-container i.fa']
    ...
    function transformLineBreaks(content) {
        content = content.replace(/\n/g, "<br />");
        return content;
    }

    function processContentForInput(content) {
        content = content.replace(/<br ?\/?>/g, "\n");

        var fragment = $('<div/>').html(content);

        // Replace links to plain text 
    ...
    function processContent(content, templates, roomCache) {
        ...
        // Keep it as false. We don't want multiline text to come in collapsible panel 
        ...
        if (hasNewline) {
            ...
            // Transform \r\n to </br>
            content = transformLineBreaks(content);
    ...
    function getPresenceText(presenceClass) {

        var langResource, offlineResource = 'ChatStatusOffline';
        switch (presenceClass) {
            case 'status-online': // Online
                langResource = 'ChatStatusOnline';
                break;
            case 'status-offline': // Offline
            case 'status-invisible': // Invisible
                langResource = offlineResource;
                break;
            case 'status-away': // Away
                langResource = 'ChatStatusAway';
                break;
            case 'status-disturb': // Busy
                langResource = 'ChatStatusDND';
                break;
            default:
                langResource = offlineResource;
                break;
        ...
        return utility.getLanguageResource(langResource);
    }
    ...
    var utility = {
        ...
        processContentForInput: processContentForInput,
        getAttachmentViewModel: function(data) {
            var attachmentViewModel = {
                ThumbUrl: data.FileThumbURL,
                Url: data.FileURL,
                Name: data.Caption,
                Extension: data.FileExtension,
                IsImage: data.IsImage,
                Size: data.FileSizeText,
                ExtensionClass: data.FileTypeIconCSSClass
            };
            return attachmentViewModel;
        },
        getUserViewModel:function(user, isOwner) {
            return {
                id: user.UserID,
                name: user.Name,
                hash: user.Hash,
                ...
                lastActive: (user.LastActivityDate) ? user.LastActivityDate.fromJsonDate():'',
                admin: user.IsAdmin,
                afk: user.IsAfk,
                presenceClass: user.Presence,
                presenceText: utility.getPresenceText(user.Presence),
                avatar: user.AvatarImageURL,
                skype: user.SkypeIM,
                onlineChatStatus: user.OnlineChatStatus
            };
        },
        getRoomViewModel: function (roomViewModel) {
            var roomName = roomViewModel.Name,
                ...
                isOwner = roomViewModel.IsOwner,
                users = roomViewModel.Users,
        ...
        appendQueryString: cfUtilities.appendQueryString,
...
})(window.jQuery, window, window.Emoji, window.marked, window.hljs, window.linkify, window.moment, Communifire.languageResources, window.Communifire);
